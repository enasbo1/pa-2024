...
@Component({
  selector: 'pm-prestate-service-detail',
  templateUrl: './prestate-service-detail.component.html',
  styleUrls: ['./prestate-service-detail.component.scss']
})
export class PrestateServiceDetailComponent implements OnInit {
  query_params?:Params;
  ...
  constructor(private serviceModelService : ServiceModelService,
              private serviceEntrepriseModelService : ServiceEnterpriseModelService,
  ...
  private delete():void{
    ...
    if (this.service_object){
      ...
      this.serviceEntrepriseModelService.delete_service(BigInt(this.service_object.id), error).subscribe(
        ()=>{
